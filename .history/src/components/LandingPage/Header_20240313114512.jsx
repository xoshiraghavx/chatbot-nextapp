    ...
    <header className="z-20 text-white bg-white bg-opacity-5">
      ...
      <nav className="mx-auto flex max-w-6xl flex-row items-center justify-between p-4">
        ...
        <div className="flex flex-row space-x-8 transition">
          <ul className="hidden flex-row space-x-6 font-medium sm:flex">
            {header_content?.nav_items &&
              header_content?.nav_items.map((item, i) => (
                <li
                  key={i}
                  className="flex cursor-pointer flex-row items-center space-x-2 hover:text-gray-800"
                >
                  <span>{item}</span>
                  <span>
                    <svg
          ...
          <div className=''>
        {/* Not signed in <br /> */}
        <button onClick={() => signIn('google')} className='cursor-pointer rounded-lg bg-black px-6 py-4 font-medium text-white hover:bg-gray-800'>Sign in</button>
      </div>
        </div>
      </nav>
    </header>
